...
- 카카오 / 네이버 / 구글 OAuth 2.0 인증 플로우 직접 구현  
  - Authorization Code 발급 → Access Token 요청 → 사용자 정보 조회  
  - Access Token 만료 시 Refresh Token을 이용한 자동 재발급 처리  
  - 각 플랫폼별 회원 탈퇴 API 연동  
- 이메일 인증 기능 구현  
  - JavaMailSender 기반 인증 메일 발송  
  - 인증번호 생성 및 유효 시간(만료) 검증 로직 작성  
  - 가입 / 비밀번호 찾기와 연동된 인증 프로세스 구성

### 👤 마이페이지 기능 구현
- 나의 게시글 / 좋아요 / 북마크 / 찜 / 팔로워·팔로잉 목록 조회  
  - MyBatis 기반 조인 쿼리 직접 작성  
- 다중 선택 삭제 기능 구현  
  - 체크박스 배열 전달 → Service 레이어에서 일괄 삭제 처리  
- 개인정보 수정 / 비밀번호 변경  
  - BCrypt 기반 비밀번호 암호화 적용  
- 회원 탈퇴 로직 구현  
- 전체 마이페이지의 페이징 처리 (Oracle ROWNUM 활용)

### 📝 커뮤니티 기능 구현
- Summernote 기반 게시글 작성 기능 구현  
  - 이미지 업로드 처리 (MultipartFile → 서버 저장 → 이미지 URL 반환)  
- 게시글 CRUD 전부 직접 구현 (등록 / 조회 / 수정 / 삭제)  
- 댓글 / 대댓글 기능  
  - jQuery + Ajax 기반 비동기 처리  
  - JSON 요청/응답 구조 설계  
- 게시글 좋아요 / 북마크 기능  
  - 토글 방식(ON/OFF) 로직 및 상태 반환 API 구현  
- 사용자 팔로우 / 언팔로우 기능  
  - 팔로우 테이블 설계 및 상태 조회 기능 구현

### 🛒 쇼핑몰 기능 구현
- 상품 찜하기 기능 구현 (토글 방식)  
- 찜한 상품 목록 조회를 위한 MyBatis SQL 작성

### 🧩 공통 / 인프라
- ERD 설계 참여  
  - 회원–커뮤니티–쇼핑몰 전체 도메인 관계 모델링  
- Spring MVC → Spring Boot 마이그레이션  
  - XML 기반 설정(Web.xml 등) 제거 및 어노테이션 기반 빈 설정 구성  
  - MVC 구조 및 패키지 구조 재정비  
- AWS EC2(Ubuntu) 배포  
