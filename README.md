...
통계청 가구 추계 자료에 따르면 1인 가구 비율은 앞으로 약 12% 증가할 것으로 전망됩니다. 이에 따라 인테리어 및 관련 제품의 소비도 더욱 증가할 것으로 예상되며, 이러한 흐름을 반영해 인테리어 쇼핑몰 프로젝트를 기획하게 되었습니다.

본 프로젝트는 인테리어에 관심 있는 사용자들이 **자유롭게 아이디어와 자신의 공간을 공유**하고, 관련 상품을 추천받아 **자연스럽게 구매까지 이어질 수 있는 커뮤니티 기반 쇼핑몰 플랫폼**을 목표로 개발되었습니다.

주요 목표는 **커뮤니티 활동을 통해 사용자들이 인테리어 아이디어를 얻고, 상품 구매까지 자연스럽게 연결되는 경험을 제공하는 것**입니다.
...
- **Google / Naver / Kakao 계정 연동 해제(탈퇴) 기능 구현**
  - Refresh Token을 활용한 Access Token 재발급 처리 로직 적용
...
- 회원 탈퇴 처리 로직 개발 
- 일부 목록 화면에 페이징을 적용하여 조회 성능 및 UX를 개선
...
- 게시글 작성 및 수정 과정에서 이미지 파일이 즉시 서버에 저장되면서, 실제 게시글에서 사용되지 않는 이미지 파일이 서버에 누적되는 문제가 발생<br>
    → 서버 저장 디렉토리 내 파일 목록과 게시글 본문에 포함된 이미지 경로를 비교하여 미사용 파일을 식별하고 자동 삭제하는 정리 로직을 구현하여 저장소 효율성을 개선
    

### 파일명 중복 문제 (UUID 적용)

- 파일 업로드 기능 구현 과정에서 동일한 파일명이 업로드될 경우 기존 파일이 덮어써지거나 충돌하는 문제가 발생<br>
    → 서버 저장 시 UUID 기반으로 파일명을 생성하여 고유성을 보장하고, 원본 파일명과 저장 파일명을 분리하여 관리하도록 개선
    

### OS 환경별 파일 경로 문제

- 파일 업로드 기능 구현 후 로컬 개발 환경(Windows)과 배포 환경(Linux/EC2) 간 파일 저장 경로 구조가 달라 이미지 업로드 및 조회가 정상적으로 동작하지 않는 문제가 발생<br>
    → OS 환경 정보를 기반으로 Windows와 Linux 환경별 파일 저장 경로를 분기 처리하고, Spring ResourceHandler를 통해 정적 리소스 접근 경로를 설정하여 개발/운영 환경 모두에서 파일 업로드 및 조회가 정상적으로 동작하도록 개선
    

### EC2 메모리 부족 (Swap 적용)

- AWS EC2 프리티어 환경에서 애플리케이션 실행 중 메모리 부족으로 인해 서버 성능 저하 및 중단 현상이 발생<br>
    → Linux 환경에서 Swap(가상 메모리)을 설정하여 부족한 메모리를 보완하고, 안정적으로 서비스가 동작하도록 서버 환경을 개선
...
- 커뮤니티와 쇼핑몰을 결합한 플랫폼을 구축해, 사용자가 콘텐츠를 보고 상품 구매까지 이어지는 **서비스 흐름을 구현**해볼 수 있었습니다.
- 개발 과정에서
  - 게시글 CRUD
  - 이메일 인증
  - Ajax 기반 댓글·대댓글
  - 소셜 로그인(OAuth 2.0) 등 핵심 기능을 구현하며 백엔드의 기본기를 다질 수 있었습니다.
