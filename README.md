...
- 게시글 작성 및 수정 과정에서 이미지 파일이 즉시 서버에 저장되면서, 실제 게시글에서 사용되지 않는 이미지 파일이 서버에 누적되는 문제가 발생
...
- 파일 업로드 기능 구현 과정에서 동일한 파일명이 업로드될 경우 기존 파일이 덮어써지거나 충돌하는 문제가 발생
...
- 파일 업로드 기능 구현 후 로컬 개발 환경(Windows)과 배포 환경(Linux/EC2) 간 파일 저장 경로 구조가 달라 이미지 업로드 및 조회가 정상적으로 동작하지 않는 문제가 발생
...
- AWS EC2 프리티어 환경에서 애플리케이션 실행 중 메모리 부족으로 인해 서버 성능 저하 및 중단 현상이 발생
