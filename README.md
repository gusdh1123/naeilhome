...
- 로컬 개발 환경(Windows)과 배포 환경(Linux/EC2) 간 파일 저장 경로 구조가 달라 이미지 업로드 및 조회가 정상적으로 동작하지 않는 문제가 발생<br>
    → OS 환경 정보를 기반으로 경로를 분기 처리하고, 정적 리소스 접근 경로를 설정하여 개발/운영 환경 모두에서 파일 업로드 및 조회가 정상적으로 동작하도록 개선
    

### EC2 메모리 부족 (Swap 적용)

- AWS EC2 프리티어 환경에서 메모리 부족으로 인해 서버 성능 저하 및 중단 현상이 발생<br>
    → Linux Swap(가상 메모리)을 설정하여 부족한 메모리를 보완하고 안정적으로 서비스가 동작하도록 서버 환경을 개선
