...
### ✉️ 이메일 인증
- 6자리 랜덤 인증번호 생성 및 이메일 발송 기능 구현
- 사용자가 입력한 인증번호와 발송된 번호를 비교하여 인증 처리

### 👤 마이페이지 기능
- 게시글 / 좋아요 / 북마크 / 찜 / 팔로워·팔로잉 목록 조회 기능 구현
- 체크박스 기반 **다중 선택 삭제** 기능 구현  <!--  - JSON 배열 수신 → Java List 변환 → 선택 항목 일괄 삭제 처리 -->
- 개인정보 수정, 비밀번호 변경 기능 구현
  - 프로필 사진 업로드 및 변경 기능 포함
- 회원 탈퇴 처리 로직 구현 
...
- 게시글 작성·수정 시 이미지가 즉시 서버에 저장되면서 실제로 사용되지 않는 파일이 누적되는 문제 발생<br>
    → 저장 디렉토리의 파일 목록과 게시글 본문의 이미지 경로를 비교해 불필요한 파일이 저장소에 쌓이지 않도록 개선

### 파일명 중복 문제 (UUID 적용)

- 동일한 파일명이 업로드될 경우 기존 파일이 덮어써지거나 충돌하는 문제가 발생<br>
    → 서버 저장 시 UUID 기반으로 파일명을 생성하여 고유성을 보장
    

### OS 환경별 파일 경로 문제

- 로컬 개발 환경(Windows)과 배포 환경(Linux/EC2) 간 파일 저장 경로 구조가 달라 이미지 업로드 및 조회가 정상적으로 동작하지 않는 문제가 발생<br>
    → OS 환경 정보를 기반으로 경로를 분기 처리하고, 정적 리소스 접근 경로를 설정하여 개발/배포 환경 모두에서 파일 업로드 및 조회가 정상적으로 동작하도록 개선
    

### EC2 메모리 부족 (Swap 적용)

- AWS EC2 프리티어 환경에서 메모리 부족으로 인해 서버 성능 저하 및 중단 현상이 발생<br>
    → Swap(가상 메모리)을 설정하여 부족한 메모리를 보완하고 서버 안정성 확보
