...
기존 Spring MVC 프로젝트를 Spring Boot로 마이그레이션
...
서비스 전반의 DB 구조를 팀 단위로 공동 설계 (ERD)

커뮤니티 및 회원 도메인 중심 테이블 구현

MyBatis 기반 SQL 매핑 및 데이터 접근 로직 작성

게시글·댓글·팔로우 등 N:M 관계 테이블 설계 및 연동

🔐 소셜 로그인

카카오 / 네이버 / 구글 OAuth 2.0 소셜 로그인 연동

인증 오류 대응 및 토큰 검증 로직 처리

🎨 UX 개선

Summernote 에디터 도입으로 자유로운 게시글 작성 환경 제공

Tiles 기반 템플릿 구성으로 공통 레이아웃 관리 및 UI 일관성 유지

📄 페이지 구현

메인 페이지, 마이페이지, 게시글 상세 페이지 등

사용자 흐름을 고려한 화면 설계 및 구현

🚀 배포 및 인프라

AWS EC2 + Docker 환경 구성

Spring Boot 프로젝트를 WAR 형태로 빌드 후 Tomcat 배포

🤝 협업 / 문서화

GitHub를 통한 형상 관리 및 이슈 관리

Notion을 활용한 문서화 및 협업 진행
